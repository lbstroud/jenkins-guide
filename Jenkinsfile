  ...
  stages {
    ...
    stage('TEST') {
      ...
      parallel {
        ...
        stage('Quality Tests') {
          steps {
            sh 'docker login --username lbstroud --password Lst@1361286'
            sh 'docker tag nodeapp-dev:trunk lbstroud/nodeapp-dev:latest'
            sh 'docker push lbstroud/nodeapp-dev:latest'
          }
        }
      ...
      steps {
        retry(count: 3) {
          timeout(time: 10, unit: 'MINUTES') {
            sh 'docker tag nodeapp-dev:trunk lbstroud/nodeapp-prod:latest'
            sh 'docker push lbstroud/nodeapp-prod:latest'
            sh 'docker save lbstroud/nodeapp-prod:latest | gzip > nodeapp-prod-golden.tar.gz'
          }
